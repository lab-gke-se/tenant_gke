...
module "gke" {
  ...
  name                       = "${local.cluster_public.cluster_type}-cluster"
  region                     = local.region
  zones                      = local.zones
  network                    = local.network_name
  network_project_id         = local.network_project_id
  subnetwork                 = local.cluster_public.subnet_name
  ip_range_pods              = local.cluster_public.pods_range_name
  ip_range_services          = local.cluster_public.svc_range_name
  ...
  enable_private_endpoint    = local.cluster_public.enable_private_endpoint
  # private_endpoint_subnetwork = local.private_endpoint_subnetwork
  master_authorized_networks = local.cluster_public.master_authorized_networks
