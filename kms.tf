...
module "prj_tenant_2_kms_key_ring" {
  ...
  name     = "${local.projects["prj_dev_tenant_2"].name}-gke-key-ring"
  project  = local.projects["prj_dev_tenant_2"].project_id
...
module "prj_tenant_2_kms_key" {
  ...
  name     = "${local.projects["prj_dev_tenant_2"].name}-gke-key"
  project  = local.projects["prj_dev_tenant_2"].project_id
  key_ring = module.prj_tenant_2_kms_key_ring.id
  services = local.prj_dev_tenant_2_services
}
